/* =========================================================================
   Agentic AI - CBT  |  Engine: Navigation, Fortschritt, Theme, Quiz
   Hinweis: Umlaute als \uXXXX-Escapes (ASCII-sicher, rendern korrekt).
   ========================================================================= */
(function () {
  "use strict";
  ...
  var MODULES = [
    ...
    { id: "fallstricke",   file: "09-fallstricke.html",   num: 9,  short: "Fallstricke" },
    { id: "produktion",    file: "10-produktion.html",    num: 10, short: "Produktion: State & Oversight" },
  ...
  var PKEY = "cbt_agentic_v4_progress";
  ...
  function buildChrome() {
    ...
    var html = '<a href="' + LANDING + '"' + (cur === null ? ' class="active"' : '') + '><span class="ix">&#8962;</span> Übersicht / Start</a>';
    ...
    if (slot && idx !== -1) {
      ...
      var prevHtml = prev
        ? '<a class="prev" href="' + prev.file + '"><span>&larr; Zurück</span><b>' + prev.num + '. ' + prev.short + '</b></a>'
        : '<a class="prev disabled"><span>&larr; Zurück</span><b>Start</b></a>';
      var nextHtml = next
        ? '<a class="next" href="' + next.file + '"><span>Weiter &rarr;</span><b>' + next.num + '. ' + next.short + '</b></a>'
        : '<a class="next" href="' + LANDING + '"><span>Fertig &rarr;</span><b>Zur Übersicht</b></a>';
  ...
  function initQuiz(quiz) {
    ...
    if (check) check.addEventListener("click", function () {
      ...
      questions.forEach(function (q) {
        ...
        opts.forEach(function (o, i) {
          ...
          var isCorrect = correct.indexOf(i) !== -1;
          var isSel = o.classList.contains("sel");
          if (isCorrect && isSel) o.classList.add("correct");
          else if (isCorrect) o.classList.add("missed");
          else if (isSel) o.classList.add("wrong");
        });
      ...
      if (resultEl) {
        resultEl.textContent = pass
          ...
          : "Knapp: " + correctCount + "/" + total + ". Schau dir die Erklärungen an und versuch es nochmal.";
    ...
    if (reset) reset.addEventListener("click", function () {
      ...
      Array.prototype.slice.call(quiz.querySelectorAll(".opt")).forEach(function (o) {
        o.classList.remove("sel", "correct", "missed", "wrong", "disabled");
      });
  ...
  /* Manuelles Abhaken (z. B. Hands-On-Modul ohne Quiz) */
  function initMarkDone() {
    Array.prototype.slice.call(document.querySelectorAll("[data-mark-done]")).forEach(function (btn) {
      var mod = currentId();
      var st = load();
      function setDone() {
        btn.disabled = true;
        btn.textContent = "Modul abgehakt ✓";
      }
      if (mod && st.done[mod]) setDone();
      btn.addEventListener("click", function () {
        if (mod) { markDone(mod); setDone(); }
      });
    });
  }
